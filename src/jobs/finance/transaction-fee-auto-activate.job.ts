...
export class TransactionFeeAutoActivateJob {
  ...
  /**
   * Auto-activate pending transaction fee rules after a cooldown window.
   *
   * - Global: activates the most recent eligible pending rule and ensures only one global is active.
   * - Channel rules: activates eligible pending rules in bulk.
   *
  ...
  async runHourly(): Promise<void> {
    ...
    const [channelRes, globalActivated] = await this.prisma.$transaction(async (tx) => {
      const channelRes = await tx.transactionFeeRule.updateMany({
        where: {
          DeletedAt: null,
          IsActive: false,
          ActivatedAt: null,
      ...
      if (!eligibleGlobal) {
        return [channelRes, false] as const;
      }
      ...
      return [channelRes, true] as const;
    });

    if (channelRes.count > 0 || globalActivated) {
      this.logger.log(
        `Auto-activated transaction fee rules (channel=${channelRes.count}, global=${globalActivated ? 1 : 0}, cutoff=${cutoff.toISOString()}, hours=${hours})`,
      );
    }
  }
}
