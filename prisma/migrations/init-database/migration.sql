-- Baseline migration (single migration) for food-delivery-server
-- Creates full schema as defined in prisma/schema.prisma
...
CREATE TABLE `ACCOUNT` (
    ...
    INDEX `ACCOUNT_EmailVerified_idx`(`EmailVerified`),
...
CREATE TABLE `ENTERPRISE` (
    ...
    `SettlementCycle` ENUM('Weekly', 'Monthly', 'Yearly') NULL,
...
CREATE TABLE `FOOD` (
    ...
    INDEX `FOOD_EnterpriseID_idx`(`EnterpriseID`),
    ...
    INDEX `FOOD_EnterpriseID_IsAvailable_idx`(`EnterpriseID`, `IsAvailable`),
...
CREATE TABLE `ORDER` (
    ...
    `Status` ENUM('Pending', 'Confirmed', 'Preparing', 'ReadyForPickup', 'OutForDelivery', 'Delivered', 'Completed', 'Cancelled', 'Refunded') NOT NULL DEFAULT 'Pending',
...
CREATE TABLE `REVIEWS` (
    ...
    INDEX `REVIEWS_IsHidden_idx`(`IsHidden`),
...
-- AddForeignKey
ALTER TABLE `SETTLEMENT` ADD CONSTRAINT `SETTLEMENT_EnterpriseID_fkey` FOREIGN KEY (`EnterpriseID`) REFERENCES `ENTERPRISE`(`EnterpriseID`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `SETTLEMENT_ITEM` ADD CONSTRAINT `SETTLEMENT_ITEM_SettlementID_fkey` FOREIGN KEY (`SettlementID`) REFERENCES `SETTLEMENT`(`SettlementID`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `SETTLEMENT_ITEM` ADD CONSTRAINT `SETTLEMENT_ITEM_OrderID_fkey` FOREIGN KEY (`OrderID`) REFERENCES `ORDER`(`OrderID`) ON DELETE RESTRICT ON UPDATE CASCADE;
